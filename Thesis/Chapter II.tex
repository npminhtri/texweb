...
For our purpose, it is enough to define the standard parabolic subgroups. Similar to the parabolic subalgebras, which is formed 
by adding some "negative" roots, we have the same situation for parabolic subgroups. It can be seen that 
the parabolic subalgebra contains elements in the form of block-upper triangular. 

Therefore, each parabolic subalgebra of $\mathfrak{g} = \slnr$ corresponds to 
a partition of $n$. In the same manner, there exists a bijection between each parabolic subgroup of $\SLn$
...
\end{pmatrix},
\]
where $m_{n_i} \in \mathrm{GL}(n_i, \mathbb{R})$ for $1 \leq i \leq r$. The integer $r$ is termed the rank of the parabolic subgroup $P_{n_1,\ldots,n_r}$.
\end{definition}
