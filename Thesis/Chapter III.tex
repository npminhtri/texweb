...
this is a lattice that achieve all the successive minima at the same time, see \cite{MR2153282}.

We will provide two different definitions of semi -stable lattice: one is geometric - which follows Grayson's notion of a canonical plot in \cite{MR780079}, and one is purely algebraic, which makes use of the maximal standard parabolic subgroups. We will show that the two definitions coincide in the next chapter.
...
In this section, we introduce Grayson's definition of \textit{semi-stable} lattices, follows \cite{MR780079}. 
Grayson associates every lattice convex hull of points in $\mathbb{R}^2$ - called its \textit{ profiles}.
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item Assume that the lemma holds for $r >1$. Consider
              \[M = \mathbb{Z}m_1\oplus \ldots \oplus\mathbb{Z}m_r\]
              is a sublattice of  $L$ of rank $r$. Consider the wedge product $\bigwedge^r L$, then clearly
              ...
              is finite to one, then we are done. Consider the sublattice $M' = L \cap (M\otimes \mathbb{Q})$. It is clear that $M' \supset M$. Assume that 
              $M'$ has a basis $\{v_1,\ldots,v_r\}$. Then we have 
              \[[M':M]v_1 \wedge \ldots \wedge v_r = m_1 \wedge \ldots \wedge m_r\]
              Thus $\bigwedge^r M$ output the index of $M$ in $M'$. As groups, there are only finite subgroups of $M'$ of a fixed index, so there are only finitely many choices of $M$ under the map $M \mapsto \bigwedge^r M$. \qedhere
    \end{itemize}
\end{proof}
\begin{definition}
...
\[||x||:= \inf\{ \sqrt{ \left\langle x+m, x+m\right\rangle}: m \in M \}\]
\begin{definition}
...
or equivalently
\begin{lemma}\cite[Corollary 4.2]{MR2127941}
...
\begin{enumerate}
    \item $L_i/L_{i-1}$ is semi-stable for all $1 \le i \le k$.
...
\begin{definition}[\label  = $\rho$-definition]\label{ss2}
    ...
    where $\text{ParSt}$ stands for the set of standard parabolic subgroup of $\GLn$ as defined in definition \ref{parst}.
\end{definition}
...
\begin{lemma}\label{ss-equiv}\citetext{\citealp[Lemma 2.2.1]{MR3969872}; \citealp[Proposition 4.2.3]{patnaik2025borel}}
    ...
    \begin{enumerate}
        ...
        \item For every standard parabolic subgroup $P\subset G$ and $\omega \in \widehat{\Delta}_P$ we have \[\left\langle \omega,H_P(x\delta)\right\rangle \ge 0\] for each $\delta \in G(\mathbb{Q})/Q_i(\mathbb{Q})$.
...
\begin{proof}
    \hfill\\
    First we prove $1 \Rightarrow 3$: This follows immediately as $\rho_Q = c\omega$ for some positive number $c$ and $\omega \in \widehat{\Delta}_Q$, by lemma \ref{rhoP}. In particular
    ...
    Finally, we have $2 \Rightarrow 1$ as $\rho_P$ is a positive linear combination of elements contained in $\widehat{\Delta}_P$, again by lemma \ref{rhoP}
\end{proof}
...
$\rho_Q$ is proportional to $\lambda_i.$ Thus $$\left\langle \rho_Q,H(B)(x\gamma)\right\rangle =0.$$ In particular, we can replace $H_Q$ by $H_B$ in verifying the semi-stability.
...
\begin{lemma}\label{canonical-pair}\citetext{\citealp[Lemma 2.3.2]{MR3969872}; \citealp[Lemma 4.2.5]{patnaik2025borel}}
    ...
    \begin{enumerate}
        ...
        \item If $(P,\delta) $ is extremal for $x$, then $\left\langle \alpha, H_P(x\delta)\right\rangle \le 0$ for any $\alpha \in \Delta_P$.\label{extremal}
    \end{enumerate}
\end{lemma}
\begin{proof}
