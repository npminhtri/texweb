...
\end{proof}
\subsection{Space of lattices}
Let $K =O_n(\mathbb{R})$ be the orthogonal subgroup of $G= \mathrm{GL}_n(\mathbb{R})$. We can identify the space $X_n =K \backslash G$ with the space of lattices 
up to rotation. Indeed, assume that we have a lattice $(L,Q)$ where 
\[ L = g\mathbb{Z}^n, \quad Q = BB^t\]
for some $g, B \in \GLn$. Define the norm $||.||_{gB}$ on $\mathbb{Z}^n$ by setting 
\[||v||_{Bg} = ||Bgv||, \quad \forall v \in \mathbb{Z}^n\]
where the norm on the right hand side of the above equation is just the standard norm. Then the lattice $(L,Q)$ and $(\mathbb{Z}^n,||.||_{Bg})$ are isomorphic. Hence each isomorphic class of lattices 
has a representative of the form $(\mathbb{Z}^n, ||.||_g)$ for some $g \in \GLn$. It can be observed that the $||.||_g = ||.||_{kg}$ for any $k \in O(n)$. This proves that $X_n$ parametric isomorphic classes of lattice bases up to rotation.
...
\begin{definition}[\label=sublattice]
    ...
    $L$ a \textbf{primitive sublattice} if and only if $L/M$ is torsion free.
\end{definition}
From this definition, we can prove that $M$ is a primitive sublattice of $L$ if it satisfies one of the
...
\end{enumerate}
We refer to the \cite[Section 1.4]{MR4505757} for a proof of these equivalences. In many references, such as \cite{MR2127941}, primitive sublattices are identified with sublattices. 
\begin{example}
...
\[||x||:= \inf\{ ||x+m||: m \in M \}\]
To verify that $L/M$ along with the given norm on the quotient group forms a lattice, we refer to \cite{MR1905312}.
\begin{definition}
