...
\begin{theorem}\label{semi-stable-equiv}
    ...
    \begin{enumerate}
        \item The bottom of the profile of $L_x$ is a line connecting  the origin and $(n,0)$.
        \item The degree of instability of $x$ is nonnegative, i.e., $\deg_{inst}(x) \ge 0$.
    \end{enumerate}
\end{theorem}
...
\begin{proof}
    ...
    Extend this basis to get a basis of $\mathbb{Z}^n$
    \[\mathfrak{C}_1 = \left\lbrace v_1,v_2,\ldots, v_n \right\rbrace \]
    In $\mathbb{Z}^n$ we have the standard basis $\mathfrak{B} = \left\lbrace e_1,e_2,\ldots,e_n\right\rbrace $, and there
    exists  $\gamma \in \glnz$ such that
    ...
    \begin{align*}
        ...
        M                                                                                        & \mapsto [\gamma]
    \end{align*}
    where $[\gamma]$ denotes the equivalence class of $\gamma$ in the quotient space. This is a well-defined map. Indeed, assume that we have a different basis
    \[ \mathfrak{C}_2 = \left\lbrace w_1,\ldots, w_n \right\rbrace\]
    where \[\mathfrak{C}' = \left\lbrace w_1,\ldots, w_i \right\rbrace \]
    is a basis for sublattice $M$. By definition \ref{lattice2}, we can regard $\mathfrak{C}, \mathfrak{C}'$ as bases of the real vector space 
    $\mathbb{M} \otimes \mathbb{R} = M_\mathbb{R}$. Similarly, $\mathfrak{C}_1, \mathfrak{C}_2$ are basis of $\mathbb{R}^n$. It is known from linear algebra that there exists a linear
    $T \colon \mathbb{R}^n \to \mathbb{R}^n$ such that $T(v_k) = w_k$ for $k =1,2,\ldots n$. Moreover, $T(M_\mathbb{R}) = M_\mathbb{R}$. Thus $T$ must have matrix representation as 
    \[[T]_\mathbb{C} = \begin{bmatrix}
        A & B \\
        0 & D
    \end{bmatrix}\]
    where $A$ is the matrix representation for $\left.T\right|_{M_\mathbb{R}}$ with respect to the bases $\mathfrak{C}$, hence is of size $i \times i$ . Note that we also have $\gamma' \cdot (\gamma)^{-1}v_k=\gamma'e_k = w_k \quad \forall k$. Thus 
    \[\gamma' \cdot (\gamma)^{-1} = \begin{bmatrix}
        A & B \\
        0 & D
    \end{bmatrix} \in Q_i(\mathbb{R}) \cap \mathrm{GL}_n(\mathbb{Z})= Q_i(\mathbb{Q}) \cap \mathrm{GL}_n(\mathbb{Z})\]
...
\begin{proof}
    ...
    the quotient $\text{GL}_n(\mathbb{Z})/(Q_i(\mathbb{Q}) \cap \text{GL}_n(\mathbb{Z})) $. This is due to the fact that $\mathrm{GL}_n$ is split and hence $\mathrm{GL}_n(\mathbb{Q}) = \mathrm{GL}_n(\mathbb{Z})P(\mathbb{Q})$ for any parabolic subgroup $P$, by \cite[Lemma 1]{MR205999}. Now let $x$ be an arbitrary lattice of rank $n$.
    ...
    $\omega \in \widehat{\Delta}_{Q_i}$, we have
    \[\left\langle \omega, H_Q(x\delta)\right\rangle = \left\langle \omega, H_B(x\delta)\right\rangle = \dfrac{n}{2}\log(a_1\cdots a_i)\]
    by equation \ref{volume - H_P} and the observation that $\widehat{\Delta}_{Q_i}$ has only one element as defined in section \ref{additional notation} . Hence
    ...
    for some positive integer $c$. It is clear that the bottom of the profile of $L_x$ is a straight line if and only if $\vol(M) \ge 0$ for any sublattice $M \subset L_x$. But this condition is equivalent 
    to $\left\langle \omega, H_B(x\delta) \right\rangle \ge 0$ for $\omega \in \widehat{\Delta}_Q$ and $Q \subset G$ a standard maximal parabolic subgroup. Using the remark in section\ref{semi-stable-verified}, we can conclude that the profile of $L_x$ has bottom as a line 
    if and only if the degree of instability of $x$ is nonnegative. \end{proof}
...
Throughout this section, we fix  $x \in X_n$ and the corresponding lattice $L_x= x\mathbb{Z}^n$.
\begin{lemma}\label{increasing-slope}
    The the following conditions are equivalent for $x \in X_n$.
    \begin{enumerate}
        \item  $\left\langle \alpha, H_P(x\delta)\right\rangle <0$ for any $\alpha \in \Delta_P$ and for some standard parabolic subgroup $P$ and $\delta \in G_\mathbb{Q}/P_\mathbb{Q}$
        \item $L_x$ has a chain of lattices
              \[\mathcal{F}: 0 = L_0 \subset M_1 \subset M_2 \subset \cdots \subset M_{k-1} \subset M_k = L_x \]
              such that $\mu(M_i/M_{i-1}) <\mu(M_{i+1}/M_i)$ for all $i$ and 
    \[\mathcal{F}\otimes \mathbb{Q} \colon L_0 \subset M_1 \otimes \mathbb{Q} \subset M_2\otimes \mathbb{Q} \subset \cdots \subset M_{k-1}\otimes \mathbb{Q} \subset M_k\otimes \mathbb{Q} =  \mathbb{Q}^n \]
    is stabilized by a rational standard parabolic subgroup $P_\mathbb{Q}$.
    \end{enumerate}
\end{lemma}
\begin{remark}
    We are not assuming $M_i/M_{i-1}$ and $M_{i+1}/M_{i}$ are semi-stable, so the chain $\mathcal{F}$ is not necessarily a canonical flag. 
\end{remark}
\begin{proof}
    Assume that the standard parabolic
    $P$ in the canonical pair is of type $(n_1,\ldots, n_k)$ as defined in subsection \ref{parabolic I}, and set
    \[d_i = n_1+n_2+\ldots +n_i\]
    In particular, the $A_P$-coordinate of $x\delta$ in the \hyperref[P-horospherical]{$P$-horospherical decomposition} is
    \begin{equation}
    ...
    Using the $P$-horospherical coordinate of section \ref{P-horospherical}, we may write
    \[x\delta = km_P(x\delta)a_P(x\delta)n_P(x\delta),\]
    where $k \in \SOn$, $n_P(x\delta) \in N_P$, $m_p(x\delta) \in M_P$ are as in section \ref{P-horospherical} and $a_P(x)$ is defined as in equation \eqref{A_P coor}. Using equation \eqref{wedge-norm}, we compute
    ...
    The second equality is because  $k\in \SOn$ preserves the length, $n_p(x\delta)$ stabilizes the flag $0 \subset \mathbb{Z}^{d_1} \subset \mathbb{Z}^{d_2} \subset \cdots \subset \mathbb{Z}^{d_k}$ and the fact the $m_p$ contains sub-block of of determinant $\pm 1$. 
    Thus,
    \begin{align*}
    ...
    By the definition of \hyperref[H-function]{($H_P$)}, we know that 
...
\begin{proof}
    ...
    \(\rho^P_R =  \rho(P)-\rho(R)\)
    which vanishes identically on $\mathfrak{a}_P$. Thus 
    \begin{align*}
...
\begin{lemma}\label{ss-slope}
    Keeping the notations $(P,\delta)$ of lemma \ref{increasing-slope}, the following conditions are equivalent
    \begin{enumerate}
        ...
        \item $x\delta$ is $P-$semi-stable.
    \end{enumerate}
\end{lemma}
\begin{proof}
By lemma \ref{P&M_P ss}, the condition $x\delta$ is $P-$semi-stable if and only if $m\overline{\delta}$ is $M_P-$semi-stable. Observe that, if $P$ is of type 
$(n_1,\ldots,n_k)$, then 
...
\subsection{Proof of theorem \ref{cp-equiv}}
