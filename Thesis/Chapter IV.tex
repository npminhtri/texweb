...
\begin{lemma}\label{useful-lemma}
    ...
    \[    \text{GL}_n(\mathbb{Z})/(Q_i(\mathbb{Q}) \cap \text{GL}_n(\mathbb{Z})) \longleftrightarrow \left\lbrace \text{ sublattice of rank $i$ of $\mathbb{Z}^n$}\right\rbrace\]\end{lemma}
\begin{proof}
...
\begin{prop}\label{cp-equiv}
    Given an $x \in X_n = K\backslash G$. Then $(P,\delta)$ is the canonical pair for $x$ if and only if $\delta P\delta^{-1}$
...
\begin{proof}
    ...
    \[x = km_P(x)a_P(x)n_P(x),\]
    where $k \in \SOn$ and $n_P(x)$ stabilizes the flag $0 \subset \mathbb{Z}^{d_1} \subset \mathbb{Z}^{d_2} \subset \cdots \subset \mathbb{Z}^{d_k}$, it is immediately that
    ...
    But this is immediate from the definition of \hyperref[H-function]{$H_P$-function} and the definition of the set of root $\Delta_P$.
\end{proof}
\begin{remark}
...
\begin{proof}
    ...
    \end{align*}
    is a bijection.    This implies that one takes the minimum over the same set in evaluating the degree of $P$-instability and $M_P$-instability.
    By \ref{H_P-decomp}, For any $R \subset P$ a standard parabolic subgroup, we have
    \[H_R(x\delta) = H_P(x\delta)+ H_{\ast R}(\text{pr}_{M_P}(x\delta)) = H_{P}(x)+ H_{\ast R}(m\overline{\delta})\]
    Note that, by definition, we have
    \[\rho^P_R =  \dfrac{1}{2}\sum_{\alpha \in \Delta^P_R}\alpha,\]
...
By \cite[Subsection 4.3.4]{patnaik2025borel}, the two conditions in lemma \ref{canonical-pair} are sufficient to guarantee that $\text{cp}(x) =(P,\delta)$. Thus $\text{cp}(x) = (P,\delta)$ if and only if
...
\end{itemize}
By theorem \ref{Grayson's criterion}, $\mathcal{F}$ is the canonicalf filtration for $L_x$.
\end{proof}