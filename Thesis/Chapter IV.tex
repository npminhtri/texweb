...
\begin{proof}
    ...
    \begin{align*}
        ...
        L_x = gM = g \bigoplus_{k=1}^i  \mathbb{Z}v_i                                            & \mapsto [\gamma]
    \end{align*}
    where $\gamma e_k = v_k$ in $\mathbb{Z}^n$ for all $ k \le i$ and $\phi_x^{-1}([\gamma]) = x \bigoplus_{k=1}^i  \mathbb{Z}(\gamma \cdot e_i)$.

    Now apply lemma \ref{ss-equiv}, we have that $\deg_{\text{inst}}(x) \ge 0$ if and only if, for every maximal parabolic subgroup $Q\subset G$ and $\omega \in \widehat{\Delta}_Q$ we have \[\left\langle \omega,H_Q(x\delta)\right\rangle \ge 0\] for each $\delta \in G(\mathbb{Q})/Q_i(\mathbb{Q})$. Note that for 
    $\omega \in \widehat{\Delta}_Q$, we have 
    \[\left\langle \omega, H_Q(x\delta)\right\rangle = \left\langle \omega, H_B(x\delta)\right\rangle\]
\end{proof}
...
\begin{proof}
    ...
    $P$ in the canonical pair is of type $(n_1,\ldots, n_k)$ and set
