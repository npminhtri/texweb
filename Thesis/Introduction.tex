...
Semi-stable lattices first appeared in the paper of Stuhler \cite{MR424707}. He was inspired 
by some work of Harder and Narasimhan on semi-stable vector bundles over curves, and studied the analogy 
...
\[0 \subset \mathbb{Z}e_1 \subset \mathbb{Z}e_1 \oplus \mathbb{Z}e_2 \subset \ldots \subset \mathbb{Z}e_1\oplus\mathbb{Z}e_2\oplus \ldots \oplus \mathbb{Z}e_n\]
...
some results in classical reduction theory without the need to use the notion of Siegel's fundamental domain. Along these lines, there are various result on semi-stability, such as 
\begin{enumerate}
    ...
    the first successive minima and the slope of the lattice. 
    \item In \cite{MR2127941}, Bill Casselman studied the partition of arithmetic groups using semi-stability. His paper hinted a connection between the partition using semistablity and Arthur's combinatorial lemma in the theory of automorphic forms.
    \item In a series of 3 papers \cite{MR3519540}, \cite{MR3268755} and \cite{MR3605031}, Uri Shapira et al. studied semi-stable lattices in the context of homogeneous dynamics. One of the 
    main results involves the relation between semi-stable lattices and well-rounded lattices, which then simplifies a conjecture of Minkowski on the geometry of number.     
\end{enumerate}
...
\begin{itemize}
    \item In chapter II, we go over the theory of two dimensional lattice. We also introduce the semistability for lattices, encoded by the upper half plane model. Using the graphical data, we show that semi-stability and $\rho-$semi-stability are the same.
    ...
    \item In chapter IV, we defined lattices in higher rank and related properties. Then semi-stable lattice and canonical filtration are studied in detail, following the papers \cite{MR780079} and \cite{MR2127941}. At the end of this chapter, $\rho$-semi-stability along with some of its basic properties are given. When the lattice is not semi-stable, each lattice is attached to a canonical filtration. Similarly, if a point $x \in \GLn$ is not $\rho$-semi-stable, we associate $x$ to a canonical pair $\mathrm{cp}(x) = (P,\delta)$.
