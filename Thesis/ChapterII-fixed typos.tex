...
\begin{definition}
    ...
    \begin{itemize}
        \item $\fh$ is a nilpotent subalgebra, i.e. $\fh^{(n)} = \underbrace{[\fh,[\fh,\ldots,[\fh,\fh]]]}_{n-\text{times}}=0$ for some integer $n >0$.

        \item It is self normalizing. Namely, we have $\fh = \left\lbrace x \in \fg : [x,\fg] \subset \fg\right\rbrace$.
    \end{itemize}
\end{definition}
A priori, it is not clear whether such a subalgebra exists for any Lie algebras. However, we have the following theorem
\begin{theorem}\cite[Corollary 1.2]{barnes1967cartan}
    Let $L$ be a Lie algebra of dimension $n$ over a field $F$ of at least $n-1$ elements. Then $L$ has a Cartan subalgebra.
\end{theorem}
In particular, any finite dimensional Lie algebra over an infinite field has a Cartan subalgebra.
\begin{example}
    ...
    \begin{equation}
        \fh = \left\lbrace H= \begin{bmatrix}
            ...
            0      & 0      & \cdots & a_n
        \end{bmatrix}, a_1 + a_2+ \ldots + a_n = 0\right\rbrace \label{A-Cartan-subalgera}
    \end{equation}
\end{example}
\begin{proof}
...
Consider a semisimple Lie algebra $\mathfrak{g}$. With respect to a choice of Cartan subalgebra $\mathfrak{h}$, we have a root space decomposition. In particular, there is a finite set
...
\begin{example}
    Let $I = \{1,2,\ldots,n-1\}$. For the simple Lie algebra $\slnr$, one choice of  Cartan subalgebra $\mathfrak{h}$ is as in \eqref{A-Cartan-subalgera}.
    With respect to this Cartan subalgebra, we can define the linear function
    \[L_i \colon \fh \to \mathbb{R}, \quad H \mapsto  a_i\]
    Then the roots are given by $\alpha_{ij} :=L_i - L_j$ for distinct $i,j \in I$. We have the root space decomposition for $\slnr$ as follows
    \[\fg = \fh \oplus \left(\bigoplus_{i \ne j}\fg_{\alpha_{ij}}\right) = \mathfrak{h}\oplus \left(\bigoplus_{i \ne j} \mathbb{R}E_{ij}\right)\]
    ...
    where the positive set of roots $\Phi_+$  comprises $\alpha_{ij}$ for $i<j$ and the remaining
    roots are in the negative set of roots $\Phi_{-}$.
...
To work with the geometry of the root system, we need the notion of the Killing form.
\begin{definition}
...
\begin{theorem}\cite[Chapter 5]{MR499562}
...
\begin{theorem}\cite[Chapter 8]{MR499562}\label{non-degenerate}
...
\end{align*}
It this notation, it can be observed that $(f_x,f_y) = (x,y)$. A more thorough treatment of the Killing form can be found in \cite[Chapter 5,8]{MR499562} .
\begin{example}\label{Killing-identification}
    Consider the lie algebra $\mathfrak{g} = \slnr$ and $\mathfrak{h}$ as in \eqref{A-Cartan-subalgera}. It can be shown that
    \[\kappa(x,y) = 2n\mathrm{tr}(xy)\]
    where $xy$ is just the usual matrix multiplication and $\mathrm{tr}$ is the trace of the matrix. We refer to \cite[Section 4.3]{MR2426516} for a detailed computation.

    \textbf{Claim:}
    \[2\dfrac{E_{i,i}-E_{i+1,i+1}}{(E_{i,i}-E_{i+1,i+1},{E_{i,i}-E_{i+1,i+1}})} = \dfrac{E_{i,i}- E_{i+1,i+1}}{2n}\]
    gives rise to the root $\alpha_i = L_i-L_{i+1}$ via the identification using Killing form.

    Indeed, by direct computation, we have
    \begin{equation}
        \left(\dfrac{E_{i,i}-E_{i+1,i+1}}{2n}, E_{j,j}-E_{j+1,j+1} \right)= \begin{cases}
            2, \text{ if } i =j    \\
            -1 \text{ if } |i-j|=1 \\
            0 \text{ otherwise }\end{cases}
    \end{equation}
    On the other hand we also have
    \[\alpha_i\left(E_{j,j}-E_{j+1,j+1}\right) = (L_i-L_{i+1})\left(E_{j,j}-E_{j+1,j+1}\right)\begin{cases}
            2, & \text{ if } i =j    \\
            -1 & \text{ if } |i-j|=1 \\
            0  & \text{ otherwise }\end{cases}\]
\end{example}
Since $\{E_{j,j}-E_{j+1,j+1}\}$ where $j \in I$ forms a basis for $\mathfrak{h}$, we must have
\[\dfrac{\left(E_{i,i}-E_{i+1,i+1},x\right)}{2n} = (L_i-L_{i+1})(x)\]
for all $x \in \mathfrak{h}$. This proves the desired claim.
\subsection{Co-roots}
...
corresponding co-root $\alpha^\vee \in \fh$ to be the element such that
\[\left\langle \beta, \alpha^\vee\right\rangle:=\beta(\alpha^\vee) = \dfrac{2(\beta,\alpha)}{(\alpha,\alpha)}\]
for all $\beta \in \fh^\ast$. It is clear from this definition that $\left\langle\alpha,\alpha^\vee \right\rangle =2$.
\begin{example}\label{simple-co-root}
    This continues the example \ref{Killing-identification}. We want to determine the co-root $\alpha_i^\vee$. It is then sufficient to compute
    $\left\langle \alpha_j, \alpha_i^\vee\right\rangle $ for $j \in I$. By definition
    \[\left\langle \alpha_j, \alpha_i^\vee\right\rangle = \dfrac{2(\alpha_j,\alpha_i)}{(\alpha_i,\alpha_i)}\]
    By example \ref{Killing-identification}, we have
    \[(\alpha_j,\alpha_i) = \dfrac{1}{4n^2}(E_{j,j}-E_{j+1,j+1},E_{i,i}-E_{i+1,i+1} ) = \begin{cases}
            \dfrac{1}{n},   & \text{ if } i = j   \\
            \dfrac{-1}{2n}, & \text{ if } |i-j|=1 \\
            0,              & \text{ otherwise}
        \end{cases}\]
    In particular,
    \[\alpha_j(\alpha_i^\vee) = \begin{cases}
            2, & \text{ if } i =j    \\
            -1 & \text{ if } |i-j|=1 \\
            0  & \text{ otherwise }
        \end{cases}\]
    This proves that $\alpha_i^\vee = E_{i,i}-E_{i+1,i+1}$.
\end{example}
\subsection{Weights and co-weights}\label{weight-algebra-level}
Fix $i \in I$, define $\lambda_i \in \fh^\ast$ by specifying
\[\left\langle \lambda_i,\alpha_j^\vee\right\rangle = \delta_{ij} = \begin{cases}
        1, i =j \\
        0, i \ne j
    \end{cases}\]
\begin{example}
    Consider again the Lie algebra $\slnr$ with the co-root $\alpha_{ij}^\vee = E_i-E_j$. Explicitly, it can be checked that $\lambda_i = L_1+L_2+\ldots+L_i$. Indeed,
    we have
    \[(L_1+L_2+\ldots+L_i)(E_{j,j}-E_{j+1,j+1}) = 1 \Leftrightarrow i = j \quad \text{ and } 0 \text{ otherwise}\]
    which uniquely characterize $\lambda_i$.
\end{example}
In the same manner we define the co-weight $\lambda_i^\vee$ such that
\[\left\langle\alpha_i,\lambda_j^\vee \right\rangle= \delta_{ij} = \begin{cases}
        1, i =j \\
        0, i \ne j
    \end{cases}\]
\begin{example}
    We work with $\slnr$. Note that
    \[(L_i - L_{i+1})(E_{1,1}+ \ldots +E_{j,j})=1 \Leftrightarrow i = j, \quad \text{ and } 0 \text{ otherwise}\]
    we can see that $\lambda_i^\vee =E_{1,1}+ \ldots +E_{i,i} $
\end{example}
...
We define the  map
\[s_\alpha(\beta):= \beta - \left\langle\beta,\alpha^\vee\right\rangle\alpha = \beta-\dfrac{2(\beta,\alpha)}{(\alpha,\alpha)}\alpha\]
where $\alpha,\beta \in \mathfrak{h}^\ast$ are roots. The space $\mathfrak{h}^\ast$ along with the bilinear map $(\cdot,\cdot)$ have the structure of a Euclidean space,
so we can  think of $s_\alpha$ as reflection transformation. When $\alpha = \alpha_i$ - the simple root - we call $s_{\alpha}$ the \textbf{simple reflection} and denote $s_{i}:=s_{\alpha_i}$.
\begin{definition}
    The Weyl group of $\slnr$ is defined to be the set generated by the simple reflections
    \[W = \left\langle s_i, i =1,2,\ldots n-1 \right\rangle \]
\end{definition}
\begin{example}
    We continue the computation as in example \ref{simple-co-root}. For any $i \in \{1,2\ldots, n-1\}$ we have
    \[s_i(L_{i+1})= L_{i+1} -\left\langle L_{i+1},\alpha_{i}^\vee\right\rangle\alpha_i = L_{i+1}-(-1)(L_i-L_{i+1})=L_i \]
    and
    \[s_i(L_i) = L_i-\left\langle L_{i},\alpha_i^\vee \right\rangle\alpha_i = L_i-(L_i-L_{i+1}) = L_i\]
    Thus, if we consider the $l+1$ dimensional space generated by $L_i, i=1,2,\ldots,n$, then the Weyl group is generated by the set of transposition $(i,i+1)$. Hence
    $W \cong S_{l+1}$ - the symmetric group of $l+1$ letters.
\end{example}
\section{Lie groups}
We only define linear Lie groups.
\subsection{General linear group and special linear group}
\begin{definition}
    A \text{general linear group} over the real field, denoted $\GLn$, is defined to be the set
    \[\GLn = \{ A \in \glnr: \det(A) \ne 0\}\]
\end{definition}
An linear Lie group is a closed subgroup of $\GLn$. The key example is the following group
\[\SLn = \{ A \in \GLn: \det(A)=1\}\]
\subsection{Relation with Lie algebras}
We first need to define exponential map
\begin{definition}
    Given a matrix $X \in \glnr$, we define the exponential of $X$ is series
    \[e^X:= I+ X+\dfrac{X^2}{2!}+ \ldots = \sum_{k=0}^\infty \dfrac{X^k}{k!}\]
\end{definition}
For each linear Lie group $G \subset \GLn$, we associate the set
\[\mathrm{Lie}(G): = \{ X \in \glnr: e^{tX} \in G, \forall t \in \mathbb{R}\}\]
It can be proven, for example see \cite[Theorem 3.2.1]{MR2426516}, that $\mathrm{Lie}(G)$ is a vector spaces. We will try to compute $\mathrm{Lie}(\GLn)$ and
$\mathrm{Lie}(\SLn)$.
\begin{example}
    We need the following result
    \begin{theorem}\cite[Section 2.1]{MR2426516}\label{exponential-matrix-trace}
        For any $X \in M_n(\mathbb{R})$, we have
        \[\det(e^X) = e^{\mathrm{tr}(X)},\]
        where $\mathrm{tr}(X)$ is just the sum of entries along the diagonal of $X$.
    \end{theorem}
    Using  Theorem \ref{exponential-matrix-trace}, it is clearly that for any $X \in M_n(\mathbb{R})$, $e^X$ is an invertible matrix, thus
    $\mathrm{Lie}(\GLn)=\glnr$. Let $X \in \slnr$ so that $\mathrm{tr}(X)=0$, and apply Theorem \ref{exponential-matrix-trace} again, we have
    \[\det(e^{tX}) = e^{t\mathrm{tr}(X)}=1,\],
    which implies $\mathrm{Lie}(\SLn) \supset \slnr$. Conversely, if $X \in \mathrm{\SLn}$, then
    \[e^{tX} \in \SLn, \forall t \in \mathbb{R} \Leftrightarrow e^{t\mathrm{tr}(X)} =\det(e^{tX}) = 1, \forall t \in \mathbb{R}\]
    Hence
    \[\mathrm{tr}(X) = \left(\dfrac{d}{dt}e^{t\mathrm{tr}(X)}\right)_0 =0\]
    which implies $\mathrm{Lie}(\SLn) \subset \slnr$. We then proved $\mathrm{Lie}(\SLn) = \slnr$.
\end{example}
\subsection{Roots at group level}
We want to understand how the roots behave at Lie group level. The analog for a Cartan subalgebra is a maximal torus given by the set
...
Then $T$ acts on $\fg$ by conjugation: $\mathrm{Ad}(t)(X):= tXt^{-1}$ for any $t \in T$ and $X \in \mathfrak{g}$. Explicitly, we can check that
\[\Ad(t)(E_{ij}) = a_ia_j^{-1}E_{ij}\]
Since every element of $T$ are diagonal and commutative, they are simultaneously diagonal. If we let $\mathfrak{g}_\chi:= \{X \in G: \mathrm{Ad}(t)(X) = \chi(t)X, \forall t\in T\}$, then
we have the decomposition
\[\mathfrak{g} = \bigoplus_{\chi \in X(T)}\mathfrak{g}_\chi,\]
where $X(T)$ is just the set of multiplicative character from the torus $T$ above to $\mathbb{R}^\ast$. This is the \textbf{root space decomposition} at Lie group level. Therefore, at the group level, the character $\chi_{ij}(\text{diag}(t_1,\ldots,t_n))=t_it_j^{-1}$
is a root whenever $i \ne j$. We define the set of  \textbf{simple roots} as
\[\Delta = \left\lbrace \chi_{i}\mid i =1,\ldots,n \right\rbrace\]
where
\[\chi_i \colon T \mapsto \mathbb{R}, t \mapsto \dfrac{t_i}{t_{i+1}}.\]
...
\[\omega_i \colon T \to \mathbb{R}^*, \quad\omega_i(t) = a_1\ldots a_i\]
\begin{remark}
    If we define the logarith map over the torus $T$ by
    \[t = \begin{bmatrix}
            a_1    & 0      & \ldots & 0      \\
            0      & a_2    & \ldots & 0      \\
            \vdots & \vdots & \ddots & \vdots \\
            0      & 0      & \cdots & a_n
        \end{bmatrix} \mapsto \log(t) := \begin{bmatrix}
            \log(a_1) & 0         & \ldots & 0         \\
            0         & \log(a_2) & \ldots & 0         \\
            \vdots    & \vdots    & \ddots & \vdots    \\
            0         & 0         & \cdots & \log(a_n)
        \end{bmatrix} \]
    Then it is clear that $\log$ is a map from $T$ to $\mathfrak{h}$ . In this way, we can think $\chi_{ij}$ as $\alpha_{ij} \circ \log$. When there are no confusion, we use the symbol $\alpha_{ij}$ to indicate
    the root at both Lie algebra level and Lie group level. Similarly we will also use $\lambda_i$ for the notation of simple weight in both Lie algebra and Lie group level as in the case of root.
\end{remark}
\begin{example}
...
\end{definition}
Clearly by lemma \ref{linear-comb-of-roots}, the weight $\lambda$ is dominant if and only if $\left\langle\lambda,\alpha_i^\vee\right\rangle \in \mathbb{Z}_{\ge 0}$ for all
...
For our purposes, it is enough to define the standard parabolic subgroups.  There exists a bijection between  standard parabolic subgroups of $\SLn$
and  partitions of $n$, and  partition $n =n_1+\ldots+n_k$ defines the standard parabolic subgroup corresponding to the partition as follows
\begin{definition}\label{parst}
    The standard parabolic subgroup $P$ of $\GLn$ associated to the partition $n = n_1 + n_2 + \cdots + n_r$ is denoted $P_{n_1,\ldots,n_r}$ and is defined to be the group of all matrices of the form
    \[
        \begin{bmatrix}
            M_{n_1} & 0       & \ldots & 0       \\
            0       & M_{n_2} & \ldots & 0       \\
...
\end{definition}
We  denote $\{e_i\}$  the standard basis of $\mathbb{R}^n$. Let
\[d_i = n_1+n_2+\ldots+n_i\]
Then any element in $P_{n_1,\ldots,n_r}$ stabilizes the chain of vector spaces
\[0 \subset \mathbb{R}^{d_1} \subset \mathbb{R}^{d_2}\subset \ldots \subset \mathbb{R}^{d_k} = \mathbb{R}^n\]
Here $\mathbb{R}^k$ is generated by the linearly independent subset $\{e_1,\ldots, e_k\}$ of the standard basis.
We call such chain of vector spaces a \textbf{flag} of type $(n_1,\ldots,n_k)$.

\begin{definition}
    The maximal standard parabolic subgroups in $\text{GL}_n(\mathbb{R})$ corresponds to the
...
There is another way to look at the parabolic subgroups
\begin{definition}[BN-pairs]
    A \textbf{BN-pairs} is a 4-tuple $(G,B,N,S)$ where $G$ is a group generated by subgroups $B$ and $N$.
    The subgroup $H = B \cap N$, $S$ is a finite set of involutions which generate the \textit{Weyl group} $W= N/H$. Moreover,
    the following axioms holds
    \begin{itemize}
...
\begin{theorem}
    ...
    \begin{itemize}
        ...
        \item All subgroups of $G$ containing $B$ arises in this way.
    \end{itemize}
\end{theorem}
...
\begin{example}\label{BN-pair-parabolic}
    ...
    \[P_{i} := P_I = \left\lbrace \begin{bmatrix}
            A & \ast \\
              & B
...
\[P_{n_1,\ldots, n_k} = \left\lbrace \begin{bmatrix}
        ...
        0      & 0      & \ldots & M_k
    \end{bmatrix} \right\rbrace\]
where $M_i \in \mathrm{GL}_{n_i}(\mathbb{R})$ is invertible of size $n_i \times n_i$.
...
\end{prop}
\begin{proof}
    The Iwasawa decomposition is essentially the Gram-Schmidt orthogonalization process. For more detail, see \cite[Subsection 10.1]{MR584623}
\end{proof}
...
\end{definition}
Using the Langlands decomposition as well as the Iwasawa decomposition $G= KAN = KB$, we obtain
the decomposition
...
This is called the \textbf{$P$-horospherical} \label{P-horospherical} decomposition of the lattice space $X_G$. In this way,
...
\end{example}
Furthermore, if $Q\supset P$ is also a parabolic subgroup, it gives rise to a parabolic subgroup $\ast P := P \cap M_Q$ of $M_Q$. In particular, we have the following result
\begin{theorem}\cite[Lemma 2]{harishchandra2006automorphic}\label{M_P - P - bijection}
    Let $P,Q$ be two  standard parabolic  subgroup such that $Q \supset P$. Then the map
    \[P \mapsto \star P := P \cap M_Q  \]
    is a bijection.
\end{theorem}
...
\begin{example}
    Consider the group $G = \mathrm{SL}_3(\mathbb{R})$. We consider the rational standard parabolic subgroup
    \[P_1 = \left\lbrace \begin{bmatrix}
            * & * & * \\
            0 & * & * \\
            0 & * & *
        \end{bmatrix} \right\rbrace \subset \mathrm{SL}_3(\mathbb{R}) \Rightarrow M_{P_1} =\left\lbrace \begin{bmatrix}
            \pm 1 & 0 & 0 \\
            0     & * & * \\
            0     & * & *
        \end{bmatrix} \right\rbrace  \]
    and $ B $ the Borel subgroup. Then
    \[\ast B = B \cap M_{P_1} = \left\lbrace \begin{bmatrix}
            1 & 0 & 0 \\
            0 & x & y \\
            0 & 0 & z
        \end{bmatrix}: xz=  1\right\rbrace
    \]
    Then it is clear that $X_{M_{P_1}}$ can be identified with the space of lattices of rank 2. The $B-$ horospherical decomposition with respect to $P_1$ is just Iwasawa decomposition in disguise.
\end{example}
...
\begin{theorem}
    Consider a semisimple Lie algebra $\mathfrak{g}$ with the root system $\Phi$ and a base $\Delta$. We denote $\Phi^+$ the set of positive roots with respect to this base. There exists a correspondence between
    ...
    \[ \Phi(P) \longleftrightarrow \mathfrak{p}:= \mathfrak{b}\oplus \bigoplus_{
            \alpha \in \Phi(P) \setminus \Phi^+} \mathfrak{g}_{\alpha}\]
\end{theorem}
\begin{proof}
    We refer to \cite[Chapter VI, \S 1.7, Proposition 20]{MR2109105} for a proof of this fact.
\end{proof}
\begin{example}
    We consider the case $\mathfrak{g} = \mathfrak{sl}_3(\mathbb{R})$. Denote
    $\Delta = \left\lbrace \alpha,\beta\right\rbrace$ base for the root system of $\mathfrak{g}$, so that
...
It is also clear that the dimension of $\mathfrak{a}(P)$ is $|J|$ as the set $\{ \alpha^\vee_i: i \in J\}  $
...
We also write $\lambda^{\vee}_{j,J}$ for the basis of $\mathfrak{a}(P)$ containing the fundamental
weight, namely $\left\langle \alpha_k,\lambda^{\vee}_{j,J}\right\rangle = \delta_{kj}$ for $j,k \in J$. We have
the following  lemma
\begin{lemma}\cite[Subsection 2.3.5]{patnaik2025borel}\label{coeff-H(J)}
...
\begin{definition}\label{H-function}
    ...
    \end{align*}
    where $\log$ is defined as in subsection \ref{Weyl-vector-def}.
\end{definition}
...
\end{proof}
\subsection{Some additional notations}\label{additional notation}
...
\end{example}
\begin{remark}
    Over $\mathfrak{a}(P)$ we can also define the notion of weight $\lambda_{i,J}$ by the relations
    \[\left\langle  \omega_j^J,\alpha_i^\vee\right\rangle=\delta_{ij}\]
    for $j\in J$. It is known that we have the relation
    \[\omega^J_i = \sum_{i \in J}c_i\alpha_j,\]
    where $c_j \ge 0$. Thus we can naturally define $\lambda_{i,J}$ on the Cartan subalgebra $\mathfrak{h}$ by letting
    $\lambda_{i,J}$ to be identically zero on $\mathfrak{a}_P$. For example, the weights in $\mathfrak{h}(J)$ are different from that of $\mathfrak{h}$. In example
...
\end{definition}
If we are given another standard parabolic subgroup $Q\subset P$ then we also set

\[\rho^P_Q:= \rho(P)- \rho(Q) = \rho_Q-\rho_P\]

We have a more concrete description for $\rho_P$ as follows
\begin{lemma}\cite[Subsection 4.1.3]{patnaik2025borel}\label{rhoP}
