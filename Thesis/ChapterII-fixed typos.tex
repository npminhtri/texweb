...
\begin{example}
    ...
    where the positive set of roots $\Phi_+$  comprises $\alpha_{ij}$ for $i<j$ and the remaining roots are in the negative set of roots $\Phi_{-}$.
\end{example}
...
\begin{example}\label{Killing-identification}
    ...
    where $xy$ is just the usual matrix multiplication and $\mathrm{tr}$ is the trace of the matrix.
...
We want to understand how the roots behave at Lie group level. The analog for the Cartan subalgebra is the maximal torus
...
\begin{example}
    ...
    $\Delta = \left\lbrace \alpha,\beta\right\rbrace$ a base for the root system of $\mathfrak{g}$, so that
