...
\end{example}
\subsection{Weights and coweights}\label{weight-algebra-level}
Similarly, we can also define the $\lambda_i$ such that
\[\left\langle \lambda_i,\alpha_j^\vee\right\rangle = \delta_{ij} = \begin{cases}
        1, i =j \\
        0, i \ne j
    \end{cases}\]
Explicitly, it can be checked that $\lambda_i = L_1+L_2+\ldots+L_i$. In the same manner we define the coweight $\lambda_i^\vee$ such that
...
\begin{definition}
    Given a root system $\Phi$ and a base $\Delta$. A \textbf{parabolic subset} $\Delta(P)$ is a subset of $\Delta$  that
    satisfies the following conditions:
    \begin{enumerate}
        \item For any $\alpha \in \Delta$, at most one of the two elements $\alpha, - \alpha$ is contained in $\Delta(P)$.
        \item It is closed, in the sense that, for any two root $\alpha,\beta \in \Delta_P$ such that $\alpha+\beta$ is a root, then $\alpha+\beta \in \Delta(P)$.
    \end{enumerate}
\end{definition}
The parabolic set parametrizes the corresponding parabolic subalgebra with the root system $\Delta$, as given in the following theorem
\begin{theorem}
    Given a semisimple Lie algebra $\mathfrak{g}$ with the root system $\Delta$. There exists a correspondence between
    parabolic subset of $\Delta(P)$ of $\Delta$ and subalgebras of $\mathfrak{g}$ containing the
    Borel subalgebra $\mathfrak{b}$. The correspondence is given by
    \[ \Delta(P) \longleftrightarrow \mathfrak{p}:= \mathfrak{b}\oplus \bigoplus_{
            \alpha \in \Delta \setminus \Delta(P)} g_{-\alpha}\]
\end{theorem}
\begin{proof}
...
\begin{example}
    ...
    $\Delta = \left\lbrace \alpha,\beta\right\rbrace$ a base for the root system of $\mathfrak{g}$. There are
    4 parabolic sets, corresponding to 4 parabolic subalgebra given as follows
    \begin{align*}
        \Delta(P) = \Delta       & \longleftrightarrow \mathfrak{p} = \mathfrak{b}                               \\
        \Delta(P) = \{\beta\}  & \longleftrightarrow \mathfrak{p} = \mathfrak{b} \oplus \mathfrak{g}_{-\alpha} \\
        \Delta(P)=  \{\alpha\} & \longleftrightarrow \mathfrak{p} = \mathfrak{b} \oplus \mathfrak{g}_{-\beta}  \\
        \Delta(P)= \varnothing   & \longleftrightarrow \mathfrak{p} = \mathfrak{g}
    \end{align*}
\end{example}
...
\end{proof}
\subsection{Roots and weights for parabolic subgroups }
We also define the set $\Delta_P: = \Delta \setminus \Delta(P)$. From  section \ref{parabolic-sets-algebra}, we can see that the parabolic subalgebras are parametrized by simple roots. If 
\[\Delta = \{\alpha_1,\ldots, \alpha_d\} = \{\alpha_i: i \in I\}\]
where $I= \{1,2,\ldots,d\}$ then each $J \subset I$ gives rise to a parabolic subalgebra $\mathfrak{p}_J$ corresponding to a parabolic subgroup $P=P_J$. In the case of $\slnr$, for each $J \subset I$ and parabolic subalgebra $\mathfrak{p} = \mathfrak{p}_J$, we just write 
\[\Delta_P = \{\alpha_i, i \notin J\} \quad, \Delta(P) = \{\alpha_i,i \in J\} \label{parabolic-roots}\]
Let $\widehat{\Delta} = \{\lambda_j, j \in I\}$  be the set of fundamental weights as in subsection \ref{weight-algebra-level}. Then for each parabolic subgroup $P= P_J$ we also define
\[\widehat{\Delta}(P) = \{{\lambda_i}_{| \mathfrak{h}(J)}, i \in J\} \quad, \widehat{\Delta}_P= \{\lambda_i, i \notin J\}\]
We give an example for the above sets we just define
\begin{example}\label{sl3-parabolic-root-weight}
    ...
    \[\Delta_{P_1} = \{\alpha_1\}, \quad \widehat{\Delta}_{P_1}= \{{\lambda_1}_{|\mathfrak{h}(J)}\}\]
\end{example}
\begin{remark}
        ...
        where $c_j \le 0$. Thus we can naturally define $\omega^J_i$ on the Cartan subalgebra $\mathfrak{h}$ by letting 
        ...
        \[{\lambda_1}_{|\mathfrak{h}(J)} = \dfrac{1}{2}\alpha_1\]
...
\begin{definition}
    Given a Parabolic subgroup $P = P_J$ of $G=\SLn$ or $G=\GLn$, we define 
    \[\rho_P = \dfrac{1}{2}\sum_{i \in J}\alpha_i\]
\end{definition}
