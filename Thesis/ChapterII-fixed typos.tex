...
\begin{example}\label{simple-co-root}
    ...
    verified directly that $\alpha_{ij}^\vee = E_{i,i} - E_{j,j}$ as
    \[\left\langle \alpha_{ij}, \alpha_{ij}^\vee \right\rangle = (L_i-L_j)(E_{i,i} - E_{j,j})=2\]
\end{example}
...
\end{example}
\begin{remark}
        Over $\mathfrak{h}(J)$ we can also define the notion of weight $\lambda_{i,J}$ by the relations
        ...
        $\lambda_{i,J}$ to be identically zero on $\mathfrak{h}_J$. For example, the weights in $\mathfrak{h}(J)$ are different from that of $\mathfrak{h}$. In example 
