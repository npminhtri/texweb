...
\begin{proof}
  ...
  Indeed, we just need to check for $\gamma$ being an inversion or translation, since these two transformations generate $\SLR$, but this is easy.
...
\begin{proof}
 If $\deg_{\text{inst}}(z)$ is achieved at some
  ...
  \[\min_{\gamma \in \Gamma/\Gamma \cap B_\mathbb{Q}}\left\langle\rho,H_B(z\gamma)\right\rangle = \left\langle \rho, H_B(z\gamma_0)\right\rangle \ge 0  \]
   Hence $\rho$-semistable locus must be the set
...
\begin{theorem}\cite[Proposition 4.2]{MR1697859}
...
\begin{proof}
  ...
  $L$ is a discrete subset of $L_\mathbb{R}$ hence an Euclidean lattice.
\end{proof}
