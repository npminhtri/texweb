\chapter{$\SL2$} % Sets article title
...
\begin{proof}
  ...
  since \( ad - bc = 1 \).
\end{proof}
The point $z = i$ is special, in the sense that its stability group is the orthogonal group $K =\SO2$. Indeed, for any \(g = \begin{bmatrix}a & b \\ c & d\end{bmatrix} \in SL_{2}(\mathbb{R})\)  we have
that
...
\end{figure}\newpage
\section{$\rho$-semi-stability of lattice}
...
\[\deg_{\text{inst}}(z):= \min_{\gamma \in  \Gamma/\Gamma \cap B} \left\langle \rho,H_B(x\gamma) \right\rangle\]
