...
\begin{proof}
 If $\deg_{\text{inst}}(z)$ is achieved at some
  ...
  \[\left\langle \rho, H_B(z\gamma_0)\right\rangle \ge 0 \]
  
  We identify $z=x+iy$ with the product $a(y)n(x)$ as above. Under the identification \ref{upper-group}, we must have
  \[\left\langle \rho, H_B(z\gamma_0) = -\log(a(\gamma_0^{-1} \circ z)) \right\rangle\]
  Assume that $\gamma_0^{-1} =
    \begin{bmatrix}
      a & b \\
      c & d
    \end{bmatrix}$, then $\gamma_0^{-1} \circ z$ satisfies
  \[\Im(\gamma_0^{-1}\circ z) = \dfrac{\Im(z)}{(cx+d)^2+(cy)^2}\]
