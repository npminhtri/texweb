...
\begin{remark}
  ...
  However, the result of this thesis follows the recent paper \cite{patnaik}, which use the inverse action. That's why we follow the same notations set up in that paper.
\end{remark}
...
\cite{grayson1}, the lattice is defined to be a module over a ring of algebraic integers, but we can always restrict to a $\mathbb{Z}$-module.
...
\end{enumerate}
We refer to the \cite{reductarith} for a proof of these equivalences.
\begin{example}
