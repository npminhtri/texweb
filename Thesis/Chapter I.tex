...
\begin{remark}
  ...
  However, the result of this thesis follows the recent paper \cite{patnaik2025borel}, which use the inverse action. That's why we follow the same notations set up in that paper.
\end{remark}
...
\subsection{Upper half plane and classification of similar lattices}
...
It is clear that this is indeed an action.  Modulo this action, we can define a bijective map as in the below lemma
\begin{lemma}\label{base-and-lattices}
  ...
  $ \text{BAS}^+/\slz$ are in bijection with the set of lattices in $\mathbb{C}$.
\end{lemma}
\begin{proof}
  ...
  Hence $ad-bc =1$ or $\begin{bmatrix}
    ...
    \end{bmatrix} \in \slz$.
\end{proof}
Note that for each element $(z,w) \in \text{BAS}^+$, we can rescaling by a factor $w$ to get a base $(z',1)$ where $\Im(z')=\Im(z/w)>0$. Since the rescaling action commutes with the action 
of $\gamma$ on $\text{BAS}^+$, it makes sense to define the space $\mathbb{C}^\times \backslash\text{BAS}^+/\slz$. By identifying the pair $(z',1)$ with $z' \in \uH$ and using lemma \ref{base-and-lattices}, we have the following bijection
\[\uH/\slz \cong \mathbb{C}^\times \backslash\text{BAS}^+/\slz \cong \mathbb{C}^\times \backslash\{\text{lattices in $\mathbb{C}$}\}\]
\begin{prop}\label{classify-similar-lattice}
...
\begin{figure}[hbt!]
  ...
  \caption{Reduction in two dimensional space}
\end{figure}
...
\subsection{Canonical plot and semistablility in two dimensional space}
...
the boundary of the convex hull of the canonical plot its \textbf{canonical polygon}. In the expository \cite{MR2127941}, Bill Casselman instead calls the canonical polygon as \textbf{profile}. We will use the terminology of Casselman.
...
we call such a lattice \textbf{unstable}. In some sense, we can see that \textbf{the degree of instability} is measured
...
\begin{lemma}
  If $L_z$ is semi-stable if and only if the lattice $L_{g \circ z}$ is semi-stable, where $g \in \SLR$.
\end{lemma}
\begin{proof}
 By definition, the lattice $L_{g\circ z}:= \mathbb{Z}(g\circ z) \oplus \mathbb{Z}$. It is obvious that $g\circ z$ and $z$ are in the same coset of the quotient space $\uH/\slz$. By Proposition \ref{classify-similar-lattice}, we have 
 \[\uH/\slz \cong \mathbb{C}^\times \backslash \{\text{lattices in $\mathbb{C}$}\}.\]
 In particular, we must have that $L_{g \circ z} = cL_{z}$ for some non-zero complex number $c$. In polar coordinate $c=re^{it}$ for some real number $t$. Let's denote $u$ the shortest vector of $L_z$. Then $cu$ is also the shortest vector of
 $cL_z$ and  
 \[|cu| = r|u|, \quad \text{ and, } \quad \vol(L_{g\circ z}) = \vol(cL_z) = c^2 \vol(L_z)\]
 Using the assumption that $L_z$ is semi-stable and the above equality, we get
\begin{align*}
  L_z \text{ is semi-stable} &\Leftrightarrow |u| \ge \sqrt{\vol(L_z)}\\
  &\Leftrightarrow |cu| \ge c\sqrt{\vol(L_z)}= \sqrt{\vol(L_{g\circ z})}\\
  &\Leftrightarrow L_{g\circ z} \text{ is semi-stable} \qedhere
\end{align*}
\end{proof}
...
\begin{figure}[hbt]
  ...
  \begin{tikzpicture}[scale =3]
    ...
    \drawCircle{4}{5}
  \end{tikzpicture}
  \caption{semi-stable locus in upper half plane}
  \label{P-partition}
\end{figure}
\section{$\rho$-semi-stability of lattices}
\subsection{Lie-theoretic definition of semi-stable lattices}
...
\begin{definition}\label{rho-definition-2-dim-lattice}
  The lattice $L_z$ corresponds to the point $z \in \uH$ is called \textbf{$\rho$-semi-stable} or just \textbf{semi-stable}
  if $\deg_{inst}(z) \ge 0$.
\end{definition}
\subsection{Semi-stable locus in the upper half plane model}
We shall use  Definition \ref{rho-definition-2-dim-lattice} to find the semi-stable locus in the upper half plane $\uH$.
\begin{prop}
  The locus of $\rho$-semi-stable points in the upper half plane $\uH$ is the
  complement of the Farey balls.
\end{prop}
\begin{proof}
 If $\deg_{\text{inst}}(z)$ is achieved at some
  $\gamma_0 \in \Gamma$ and $z$ is $\rho$-semi-stable, then
  \[\min_{\gamma \in \Gamma/\Gamma \cap B_\mathbb{Q}}\left\langle\rho,H_B(z\gamma)\right\rangle = \left\langle \rho, H_B(z\gamma_0)\right\rangle \ge 0  \]
   Hence $\rho$-semi-stable locus must be the set
  ...
  Note that, for any pair of co-prime integers $(c,d)$, we can always find integers $a,b$ such that $ad-bc =1$ by Bezout's theorem. Assume that $\gamma^{-1} =
    \begin{bmatrix}
      a & b \\
      c & d
    \end{bmatrix} \in \slz$, then $\gamma_0^{-1} \circ z$ satisfies
  \[\Im(\gamma^{-1}\circ z) = \dfrac{\Im(z)}{(cx+d)^2+(cy)^2}\]
  Thus, $z$ is semi-stable if and only if it satifies the inequalities
  \[y \le(cx+d)^2+(cy)^2  \Leftrightarrow \left(x+\dfrac{c}{d}\right)^2+\left(y-\dfrac{1}{2c^2}\right)^2 \ge \left(\dfrac{1}{2c^2}\right)^2. \]
   Since the above equation is exactly the equation for Farey balls, we are done.
\end{proof}
...
\begin{definition}[\label = Abstract $\mathbb{Z}$-lattices]
  ...
  of finite rank. Suppose that $L$ is endowed with a real-valued  positive definite\footnote{The non-degenerate implicity state that rank L is the same as $\text{dim} L_\mathbb{R}$} quadratic form $Q\colon L \to \mathbb{R}$, such that
