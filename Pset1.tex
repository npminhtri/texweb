...
\begin{enumerate}
    \item
          Prove that if $f$ is an entire function, then $f(\BC)$ is dense in $\BC$.

    \item Consider the series
          \[ \sum_{k=0}^\infty z^{2^k} = z+z^2+ z^4+\ldots\]
          Determine the singular points and the natural boundary (if exists) of the above series.
    \item
          Show that for any injective analytic map $f \colon \BC \to \BC$, it must have the form
          \[ f(z) = az + b, \quad a \neq 0, b \in \BC\]
          Similarly, show that all injective maps from $\BC_\infty \to \BC_\infty$ has the form
          \[ f(z) = \frac{az+b}{cz+d}, \]
          where $ad - bc \neq 0$.
\end{enumerate}
\end{document}